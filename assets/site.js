(() => {
  ...
  const consent = localStorage.getItem('koji_analytics_consent');
  const consentBox = $('#consent');
  if (consent === 'accepted') loadAnalytics();
  else if (!consent && consentBox) consentBox.classList.add('show');

  $('#acceptCookies')?.addEventListener('click', () => {
    localStorage.setItem('koji_analytics_consent', 'accepted');
    consentBox?.classList.remove('show');
    loadAnalytics();
  });

  $('#declineCookies')?.addEventListener('click', () => {
    localStorage.setItem('koji_analytics_consent', 'declined');
    consentBox?.classList.remove('show');
  });
